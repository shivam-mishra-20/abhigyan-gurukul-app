  ...
  const renderQuestionInput = () => {
    ...
    switch (question.type) {
            ...
            {question.options?.map((opt, idx) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <Text className={`font-bold text-base ${isSelected ? "text-white" : "text-gray-600"}`}>
      ...
      case "multi_select":
        ...
        return (
          <View className="mt-4">
            <Text className="text-gray-500 text-sm mb-3 italic">Select all that apply</Text>
            {question.options?.map((opt, idx) => {
        ...
        return (
          <View className="mt-4">
            <View className="bg-emerald-50 rounded-xl p-4 mb-4 border border-emerald-200">
              <Text className="text-emerald-700 font-semibold mb-2">Assertion (A)</Text>
              <MathText text={question.assertionText || ""} fontSize={15} />
            </View>
            <View className="bg-purple-50 rounded-xl p-4 mb-4 border border-purple-200">
              <Text className="text-purple-700 font-semibold mb-2">Reason (R)</Text>
      ...
      default:
        return (
          <View className="mt-4 p-4 bg-gray-100 rounded-xl">
            <Text className="text-gray-600">This question type is not yet supported on mobile.</Text>
          </View>
        );
    }
  };
      ...
      <View className="bg-white border-b border-gray-200 px-4 py-3">
        <View className="flex-row items-center justify-between">
          <View className="flex-1">
            ...
            <Text className="text-gray-500 text-sm">{currentSection?.title || "Section"}</Text>
          </View>
          <View className="flex-row items-center">
            ...
            >
              ...
              <Text className="font-bold ml-1.5" style={{ color: getTimerColor() }}>
        ...
        {saving && (
          <View className="flex-row items-center mt-2">
            ...
            <Text style={{ color: THEME.primary }} className="text-xs ml-2">Saving...</Text>
          </View>
        )}
      </View>
        ...
        <View className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          ...
          <View className="flex-row items-center justify-between mb-4">
            ...
            >
              ...
              />
              <Text className={`ml-1.5 text-sm font-medium ${isMarked ? "text-amber-600" : "text-gray-600"}`}>
      ...
      <View className="bg-white border-t border-gray-200 px-4 py-3">
        <View className="flex-row items-center justify-between">
          ...
          >
            ...
            <Text className={`font-semibold ml-1 ${isFirstQuestion ? "text-gray-400" : "text-gray-700"}`}>
              Previous
            </Text>
          </Pressable>
          ...
          >
            ...
            <Text style={{ color: THEME.primary }} className="font-semibold ml-1.5">
          ...
          <Pressable
            ...
            style={{ backgroundColor: isLastQuestion ? "#f3f4f6" : THEME.primary }}
          >
            <Text className={`font-semibold mr-1 ${isLastQuestion ? "text-gray-400" : "text-white"}`}>
              Next
            </Text>
            <Ionicons name="chevron-forward" size={20} color={isLastQuestion ? "#9ca3af" : "white"} />
          </Pressable>
        </View>
      </View>
      ...
      <Modal visible={showPalette} animationType="slide" transparent>
        <View className="flex-1 bg-black/50 justify-end">
          <View className="bg-white rounded-t-3xl p-6 max-h-[70%]">
            <View className="flex-row items-center justify-between mb-4">
              <Text className="text-gray-900 text-xl font-bold">Question Navigator</Text>
            ...
            <View className="flex-row flex-wrap gap-3 mb-4">
              <View className="flex-row items-center">
                <View className="w-4 h-4 rounded" style={{ backgroundColor: THEME.primary }} />
                <Text className="text-gray-600 text-sm ml-1.5">Answered</Text>
              </View>
              <View className="flex-row items-center">
                <View className="w-4 h-4 rounded bg-gray-200" />
                <Text className="text-gray-600 text-sm ml-1.5">Not Answered</Text>
              </View>
              <View className="flex-row items-center">
              ...
              {data.sections.map((section, sIdx) => {
                ...
                return (
                  <View key={section._id} className="mb-4">
                    <Text className="text-gray-700 font-semibold mb-2">{section.title}</Text>
                        ...
                        return (
                          <Pressable
                            ...
                            style={{
                              ...
                              borderColor: isCurrent ? THEME.primaryDark : "transparent",
                            }}
                          >
                            <Text
                              className="font-bold"
                              style={{ color: isAnswered && !isReviewMarked ? "white" : "#374151" }}
                            >
                              {gIdx + 1}
                            </Text>
                          </Pressable>
        ...
        <View className="flex-1 bg-black/50 items-center justify-center p-6">
          <View className="bg-white rounded-2xl p-6 w-full max-w-sm">
            <View className="items-center mb-4">
              ...
              <Text className="text-gray-900 text-xl font-bold">Submit Exam?</Text>
            </View>
              ...
              return (
                <View className="bg-gray-50 rounded-xl p-4 mb-4">
                  <View className="flex-row justify-between mb-2">
                    ...
                    <Text style={{ color: THEME.primary }} className="font-bold">{stats.answered}</Text>
                  </View>
                  <View className="flex-row justify-between mb-2">
                    <Text className="text-gray-600">Unanswered</Text>
                    <Text className="text-red-500 font-bold">{stats.unanswered}</Text>
                  </View>
                  <View className="flex-row justify-between">
                    <Text className="text-gray-600">Marked for Review</Text>
                    <Text className="text-amber-500 font-bold">{stats.marked}</Text>
                  </View>
                </View>
              );
            })()}
