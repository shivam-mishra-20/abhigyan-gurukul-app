...
export default function LeaderboardScreen() {
  ...
  const getRankStyle = (rank: number) => {
    if (rank === 1) return { bg: "#fef3c7", border: "#fbbf24", icon: "trophy", color: "#b45309" };
    if (rank === 2) return { bg: "#f3f4f6", border: "#9ca3af", icon: "medal", color: "#6b7280" };
    if (rank === 3) return { bg: "#fef3c7", border: "#d97706", icon: "medal", color: "#92400e" };
            ...
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center">
                <View className="bg-white/20 w-12 h-12 rounded-full items-center justify-center mr-4">
                  <Text className="text-white text-xl font-bold">#{data.myRank.rank}</Text>
                </View>
                <View>
                  <Text className="text-white font-bold text-lg">{data.myRank.name}</Text>
              ...
              <View className="items-end">
                <Text className="text-white text-2xl font-bold">{data.myRank.avgPercentage}%</Text>
                ...
                >
                  ...
                  >
                    {style.icon ? (
                      ...
                      <Text className="font-bold" style={{ color: style.color }}>
                        {entry.rank}
                      </Text>
                    )}
                  </View>
                  ...
                  <View className="flex-1">
                    <Text className="text-gray-900 font-bold">{entry.name}</Text>
                    <View className="flex-row items-center mt-0.5">
                      {entry.classLevel && (
                        <Text className="text-gray-500 text-sm">Class {entry.classLevel}</Text>
                      )}
                      <Text className="text-gray-400 text-sm"> • {entry.examsTaken} exams</Text>
                    </View>
                  </View>

                  {/* Score */}
                  <View className="items-end">
                    <Text className="text-emerald-600 text-lg font-bold">{entry.avgPercentage}%</Text>
