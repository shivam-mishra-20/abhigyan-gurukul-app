...
import { createHomework, Homework } from "@/lib/enhancedApi";
...
export default function CreateHomeworkScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [classData, batchData] = await Promise.all([
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(
        `Homework ${form.status === "published" ? "published" : "saved as draft"}!`,
      );
      router.back();
    } catch (error: any) {
        ...
        <View className="bg-white border-b border-gray-200 px-4 py-4">
          <View className="flex-row items-center justify-between">
            ...
            <View className="bg-emerald-100 px-3 py-1 rounded-full">
              <Text className="text-emerald-700 text-xs font-medium">New</Text>
            </View>
          </View>
        </View>
          ...
          <View className="flex-row gap-3 mt-2">
            <Pressable
              ...
              className={`flex-1 py-4 rounded-xl items-center flex-row justify-center ${
                loading || uploading ? "bg-gray-100" : "bg-gray-200"
              }`}
            >
